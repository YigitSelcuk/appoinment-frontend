...
const ContactsTable = () => {
  ...
  const fetchContacts = async () => {
    try {
      ...
      showError("Kişiler yüklenirken hata oluştu");
      setContacts([]);
    } finally {
      setLoading(false);
    }
  ...
  // Sayfa veya debounced arama terimi değiştiğinde kişileri yükle
  useEffect(() => {
    fetchContacts();
  ...
  const handleContactAdded = () => {
    fetchContacts(); // Yeni kişi eklendikten sonra listeyi yenile
  };
  ...
  const handleContactUpdated = () => {
    fetchContacts(); // Kişi güncellendikten sonra listeyi yenile
  };

  const handleContactDeleted = () => {
    fetchContacts(); // Kişi silindikten sonra listeyi yenile
  };
  ...
  const handleExcelImportComplete = () => {
    fetchContacts(); // Yeni veriler eklendikten sonra listeyi yenile
  };
  ...
  const handleBulkSMSSend = async (smsData) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setSelectedContacts([]); // Seçimleri temizle
        setSelectAll(false);
        closeAllModals();
      } else {
                ...
                <tr key={contact.id}>
                  ...
                  <td>
                    <ContactActionMenu
                      ...
                      onWhatsApp={() => handleWhatsAppMessage(contact)}
...
const ContactActionMenu = ({ onView, onSendMessage, onWhatsApp, onEdit, onDelete }) => {
